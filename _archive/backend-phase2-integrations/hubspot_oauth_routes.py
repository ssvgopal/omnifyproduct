...
from integrations.hubspot.oauth2 import hubspot_oauth2

logger = logging.getLogger(__name__)

router = APIRouter(prefix="/api/integrations/hubspot", tags=["HubSpot OAuth"])
...
async def get_authorization_url(
    ...
    try:
        ...
        if not hubspot_oauth2.client_id:
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail="HubSpot OAuth2 not configured. Set HUBSPOT_CLIENT_ID environment variable."
            )
        ...
        auth_url = hubspot_oauth2.get_authorization_url(state=state)
        ...
        logger.error(f"Failed to generate HubSpot OAuth URL: {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=str(e)
        )
...
async def handle_oauth_callback(
    ...
    try:
        ...
        try:
            tokens = await hubspot_oauth2.exchange_code_for_tokens(request.code)
        except Exception as e:
        ...
        logger.info(f"HubSpot OAuth connected for organization {organization_id}")
        ...
        logger.error(f"Failed to handle HubSpot OAuth callback: {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def refresh_access_token(
    ...
    try:
        ...
        tokens = await hubspot_oauth2.refresh_access_token(credentials["refresh_token"])
        ...
        logger.error(f"Failed to refresh HubSpot token: {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
