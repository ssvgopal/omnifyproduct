...
from integrations.tiktok.oauth2 import TikTokAdsOAuth2

logger = logging.getLogger(__name__)

router = APIRouter(prefix="/api/integrations/tiktok-ads", tags=["TikTok Ads OAuth"])
...
async def get_authorization_url(
    ...
    try:
        ...
        redirect_uri = os.environ.get("TIKTOK_REDIRECT_URI", "https://app.omnify.com/integrations/tiktok-ads/callback")
        
        if not client_id or not client_secret:
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail="TikTok Ads OAuth2 not configured"
            )
        ...
        oauth2 = TikTokAdsOAuth2(client_id, client_secret, redirect_uri)
...
async def handle_oauth_callback(
    ...
    try:
        ...
        oauth2 = TikTokAdsOAuth2(client_id, client_secret, redirect_uri)
        try:
            tokens = await oauth2.exchange_code_for_tokens(request.code, request.auth_code)
        except Exception as e:
            logger.error(f"TikTok Ads token exchange failed: {e}", exc_info=True)
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
        ...
        if not tokens or not tokens.get("access_token"):
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail="Invalid token response from TikTok Ads"
            )
        ...
        logger.error(f"Unexpected error in TikTok Ads OAuth callback: {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    try:
        ...
        if not integration:
            raise HTTPException(
                ...
                detail="TikTok Ads integration not found"
            )
        ...
        try:
            ...
            logger.error(f"TikTok Ads token refresh failed: {e}", exc_info=True)
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
